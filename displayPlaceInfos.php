...
if(isset($_GET['id'])){
    $id = intval($_GET['id']);
    $sql_query = "SELECT DISTINCT scs_places.id as id, scs_places.name as placeName, lat, lng, description, website, facebook, twitter, scs_categories.name as category FROM scs_places, scs_categories WHERE scs_places.id_category=scs_categories.id AND scs_places.id=$id";
